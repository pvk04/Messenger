...
import putViewMessages from "../../../fetches/putViewMessages.fetch";
...
function ChatItem({
	...
	const [{ accountId }, dispatch] = React.useContext(AppContext);

	function chooseChat() {
		dispatch({ type: "SET_PERSON", payload: personId });
		putViewMessages(href, accountId);
	}
	...
	return (
		<NavLink
			className={({ isActive }) =>
				isActive
					? `${styles.item_link} ${styles.item_active}`
					: styles.item_link
			}
			to={`${href}`}
			onClick={chooseChat}
				...
				<div className={styles.item}>
					<header>
						...
						<div className={styles.item_head}>
							...
							<p className={styles.item_head_p}>
								{formatDate(date)}
							</p>
						</div>
					</header>
